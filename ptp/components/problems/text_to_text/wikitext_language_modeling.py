...
class WikiTextLanguageModeling(Problem):
    ...
    def __init__(self, name, params):
        ...
        if self.params['dataset'] == "wikitext-2":
        ...
        if not io.check_files_existence(os.path.join(self.data_folder, self.params['dataset']), filenames):
            ...
            filenames = ['wiki.train.tokens', 'wiki.valid.tokens', 'wiki.test.tokens']
            io.move_files_between_dirs(self.logger, os.path.join(self.data_folder, self.params['dataset']) , self.data_folder, filenames)

        # Pick (sub)set: train/valid/test.
        ...
        self.sentences = io.load_string_list_from_txt_file(self.data_folder, "wiki."+subset+".tokens")
        self.logger.info("Loaded {} sentences from the '{}' subset".format(len(self.sentences), subset))

        # Create a single text by rereplacing newlines with <eos> tokens.
        self.text = " <eos> ".join(self.sentences)

        self.logger.info("Loaded set of size {}. Example:\n  '{}'".format(len(self.sentences), self.text[0:100]))
        exit(1)
    ...
    def output_data_definitions(self):
        ...
        return {
            ...
            self.key_inputs: DataDefinition([-1, 1], [list, str], "Batch of input sentences, each consisting of several words [BATCH_SIZE] x [string CONTEXT_SIZE * WORD]"),
    ...
    def __getitem__(self, index):
        ...
        :return: ``DataDict({'indices', inputs','targets'})``

        """
        # Return data_dict.
        data_dict = self.create_data_dict(index)
        data_dict[self.key_inputs] = ' '
        data_dict[self.key_targets] = ' '
